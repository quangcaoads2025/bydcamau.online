...
(function () {
  ...
  function updateSeo(vehicle) {
    ...
    if (canonical) canonical.href = `${CONFIG.domain}/san-pham/?slug=${vehicle.slug}`;
    if (ogTitle) ogTitle.content = `${vehicle.name} | BYD Cà Mau`;
    if (ogDescription) ogDescription.content = description;
    qs('meta[property="og:url"]')?.setAttribute('content', `${CONFIG.domain}/san-pham/?slug=${vehicle.slug}`);
  ...
  function renderVehiclePage(vehicle) {
    ...
    target.innerHTML = `
      <section class="vehicle-breadcrumb container" aria-label="Breadcrumb">
        <a href="/">Trang chủ</a><span>/</span><a href="/#vehicles">Dòng xe</a><span>/</span><strong>${vehicle.name}</strong>
      </section>
    ...
    const schemas = [
      {
        ...
        url: `${CONFIG.domain}/san-pham/?slug=${vehicle.slug}`,
      },
      {
        '@context': 'https://schema.org',
        '@type': 'BreadcrumbList',
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: vehicle.name, item: `${CONFIG.domain}/san-pham/?slug=${vehicle.slug}` }
        ]
      }
    ];
