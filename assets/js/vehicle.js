...
(function () {
  ...
  function createVehicleSchema(vehicle) {
    ...
    const productSchema = {'@context':'https://schema.org','@type':['Product','Vehicle'],name:vehicle.name,image:[image],description:vehicle.shortDescription,brand:{'@type':'Brand',name:'BYD'},manufacturer:{'@type':'Organization',name:'BYD'},fuelType:vehicle.powertrain === 'EV' ? 'Electric' : 'Plug-in Hybrid',vehicleConfiguration:vehicle.segment,url:`${CONFIG.domain}/san-pham/${vehicle.slug}/`,offers:vehicle.price ? {'@type':'Offer',priceCurrency:'VND',price:vehicle.price,availability:'https://schema.org/InStock',url:`${CONFIG.domain}/san-pham/${vehicle.slug}/`} : undefined};
