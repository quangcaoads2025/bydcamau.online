...
(function () {
  ...
  function updateSeo(article) {
    ...
    qs('link[rel="canonical"]')?.setAttribute('href', `${CONFIG.domain}/tin-tuc/?slug=${article.slug}`);
    ...
    qs('meta[property="og:url"]')?.setAttribute('content', `${CONFIG.domain}/tin-tuc/?slug=${article.slug}`);
  ...
  function renderArticle(article) {
    ...
    target.innerHTML = `
      <div class="article-breadcrumb container" aria-label="Breadcrumb">
        <a href="/">Trang chủ</a><span>/</span><a href="/tin-tuc/">Tin tức</a><span>/</span><strong>${article.title}</strong>
      </div>
    ...
    const schemas = [
      {
        ...
        mainEntityOfPage: `${CONFIG.domain}/tin-tuc/?slug=${article.slug}`
      },
      {
        '@context': 'https://schema.org',
        '@type': 'BreadcrumbList',
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Tin tức', item: `${CONFIG.domain}/tin-tuc/` },
          { '@type': 'ListItem', position: 3, name: article.title, item: `${CONFIG.domain}/tin-tuc/?slug=${article.slug}` }
        ]
      }
    ];
